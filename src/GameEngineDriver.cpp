 ...
 * It creates a console-driven interface that allows the user to navigate through all states
 ...
 * It demonstrates turn-based gameplay using states:
 *   - "play",
...
/**
* Automated test for Main Game Loop
* It demonstrates:
...
*/
void testAutomaticMainGameLoop() {
  std::cout << "\n======================================\n";
  ...
  if (player1->getOrders()->orders->size() > 0) {
    ...
    if (deployOrder) std::cout << "Order is confirmed to be a Deploy order\n";
  }
  ...
  if (player2->getOrders()->orders->size() > 0) {
    ...
    if (advanceOrder) std::cout << "Order is confirmed to be an Advance order\n";
  }
  ...
  for (auto& terr : testMap->getTerritories()) {
    terr->setOwner(player1);
  }
